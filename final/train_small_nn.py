...
x_skb_select_train = np.load("data/select_k_best_train.npy")
x_skb_select_test = np.load("data/select_k_best_test.npy")
y_train = np.load("data/y_train.npy")
y_test = np.load("data/y_test.npy")

model = neural_network(model_prefix="select_k_best",nodes=[64,16], dropouts = [0.1,0])
model.fit(x_skb_select_train, y_train, x_skb_select_test, y_test,
          n_epoch=20,epoch=1, bs=128)

#
# all_model = neural_network(model_prefix="all_data",nodes=[64,16], dropouts = [0.1,0])
